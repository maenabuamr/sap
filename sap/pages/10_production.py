...
@st.cache_data
def load_materials():
    df = pd.read_csv("data/production_materials.csv")
    df["OrderDate"] = pd.to_datetime(df["OrderDate"], format="%m/%d/%Y", errors="coerce")
    df["Year"] = df["OrderDate"].dt.year
    df["Month"] = df["OrderDate"].dt.month
    return df
...
tab1, tab2, tab3, tab4, tab5, tab6 = st.tabs(["1- انتاج شهري", "2- استهلاك مواد", "3- الهدر", "4- صنف محدد", "5- الكفاءة", "6- التكلفة"])
...
st.divider()
st.caption("ERP AI Analytics | Production V1.0")
